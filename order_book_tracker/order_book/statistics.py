...
def calculate_statistics(period_type: int):
    ...
    result = OrderBook.objects.filter(timestamp__gte=start_date).aggregate(min_price=Min("last_price"),
                                                                           max_price=Max("last_price"),
                                                                           avg_price=Avg("last_price"),
                                                                           total_volume=Sum("last_size"))

    try:
        statistic = OrderBookStatistics.objects.filter(created_at__gte=start_date, period_type=period_type).first()
        ...
        OrderBookStatistics.objects.create(period_type=period_type,
