...
from order_book_tracker.order_book.tasks import (
...
def test_get_order_book_data(mocker: MockerFixture):
    success_mock_response = mocker.Mock(
        **{
            "json.return_value": {
                "status": "success",
                "data": {
    ...
    mocker.patch(
        "order_book_tracker.order_book.tasks.requests.get",
        return_value=success_mock_response,
    )
    get_and_save_order_book_data()
