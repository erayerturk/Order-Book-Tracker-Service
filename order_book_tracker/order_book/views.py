...
class OrderBookStatisticsAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if period_type in PeriodType.labels:
            return OrderBookStatistics.objects.filter(period_type=PeriodType.labels.index(period_type))
        raise ValidationError(f"Not valid period type. Valid options: {PeriodType.labels}")
